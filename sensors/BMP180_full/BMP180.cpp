...
/** Side note
*   The pressure is measured in hectopascal(100 Pa)
*
...
BMP180::BMP180(PinName sda, PinName scl, int address): m_i2c(sda,scl), m_addr(address){
    m_altitude = 0;
...
BMP180::BMP180(I2C& i2c, int address) : m_i2c(i2c), m_addr(address){
    m_altitude = 0;
    m_oss = BMP180_OSS_NORMAL; 
    m_temperature = UNSET_BMP180_TEMPERATURE_VALUE;
    m_pressure = UNSET_BMP180_PRESSURE_VALUE;  
...
int BMP180::ReadPressureData(float* pPressure){
    long p;
...
int BMP180::ReadTemperatureData(float* pTemperature){
    long t;
    if (!!ReadRawTemperature(&t)){
        m_temperature = UNSET_BMP180_TEMPERATURE_VALUE;
        return 0;
    }
    m_temperature = TrueTemperature(t);
...
int main() {
    pc.baud(9600);
    pc.printf("Initaliziing \r\n");
    BMP180 bmp180(PIN_SDA, PIN_SCL);
    float pressure, temperature;
    pc.printf("Still Initializing \r\n");
    ...
    while(1) {        
        if (bmp180.ReadData(&temperature, &pressure)){
            printf("Pressure(hPa) - 1000 (hPa): %2.4f \t Temperature(C): %8.2f\n", (pressure-1000), temperature);
        }
        wait(1);
     }
 }